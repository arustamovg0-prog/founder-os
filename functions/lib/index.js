...
exports.founderWeeklyDigest = exports.onStartupMetricUpdate = exports.aiRoadmapHints = exports.onUserCreated = exports.onPitchFeedback = exports.onStageVerified = exports.investorMatchEngine = exports.onPitchDeckUploaded = void 0;
...
async function writeNotification(userId, notif) {
    await db.collection('notifications').add(Object.assign(Object.assign({ userId }, notif), { read: false, createdAt: admin.firestore.FieldValue.serverTimestamp() }));
}
...
// ─────────────────────────────────────────────────────────────────────────────
exports.onPitchDeckUploaded = functions
    .region('us-central1')
...
`;
        ...
        if (((_a = startupData.founderIds) === null || _a === void 0 ? void 0 : _a.length) > 0) {
...
// ─────────────────────────────────────────────────────────────────────────────
exports.investorMatchEngine = functions
    .region('us-central1')
    ...
    try {
        ...
        const investors = investorsSnap.docs.map(d => (Object.assign({ id: d.id }, d.data())));
        ...
        const startupsSnap = await db.collection('startups')
            ...
            .limit(20)
            .get();
        const startups = startupsSnap.docs.map(d => (Object.assign({ id: d.id }, d.data())));
...
- Focus areas: ${((_a = investor.focusAreas) === null || _a === void 0 ? void 0 : _a.join(', ')) || 'General tech'}
...
${i + 1}. ${s.name} (${s.industry}, ${s.stage})
   AI Score: ${((_a = s.aiScores) === null || _a === void 0 ? void 0 : _a.overallReadinessScore) || 'N/A'}/100
   MRR: $${((_c = (_b = s.metrics) === null || _b === void 0 ? void 0 : _b.mrr) === null || _c === void 0 ? void 0 : _c.toLocaleString()) || 0}
   MAU: ${((_e = (_d = s.metrics) === null || _d === void 0 ? void 0 : _d.mau) === null || _e === void 0 ? void 0 : _e.toLocaleString()) || 0}
   Location: ${s.location}
...
`;
            ...
            if (!((_b = matchData === null || matchData === void 0 ? void 0 : matchData.topMatches) === null || _b === void 0 ? void 0 : _b.length))
                continue;
            ...
            if (investor.email) {
                const topHtml = matchData.topMatches.map((m, i) => `
...
// ─────────────────────────────────────────────────────────────────────────────
exports.onStageVerified = functions
    .region('us-central1')
    .firestore
    ...
    if (!startupDoc.exists)
        return null;
    const { name: startupName, founderIds = [] } = startupDoc.data();
...
// ─────────────────────────────────────────────────────────────────────────────
exports.onPitchFeedback = functions
    .region('us-central1')
    ...
    if (before.status === after.status && before.feedback)
        return null;
    if (!after.feedback || after.status !== 'feedback_submitted')
        return null;
...
`;
        ...
        if (analysis) {
            ...
            // Уведомляем фаундера
            if ((_a = after.founderIds) === null || _a === void 0 ? void 0 : _a.length) {
...
exports.onUserCreated = functions
    ...
    try {
        ...
        const role = (userData === null || userData === void 0 ? void 0 : userData.role) || 'founder';
        // Устанавливаем Custom Claims
        await admin.auth().setCustomUserClaims(user.uid, {
            role,
            linkedStartupId: (userData === null || userData === void 0 ? void 0 : userData.linkedStartupId) || null,
        });
        ...
        for (const adminDoc of adminsSnap.docs) {
            await writeNotification(adminDoc.id, {
                ...
                body: `${(userData === null || userData === void 0 ? void 0 : userData.displayName) || user.email} зарегистрировался как ${role}`,
                href: '/admin/startups',
            });
        }
...
// ─────────────────────────────────────────────────────────────────────────────
exports.aiRoadmapHints = functions
    .region('us-central1')
    ...
    const snap = await db.collection('startups')
        ...
        .limit(30).get();
    if (snap.empty) {
        log('aiRoadmapHints', 'No stuck startups');
        return null;
    }
    const model = gemini.getGenerativeModel({ model: 'gemini-1.5-flash' });
    for (const startupDoc of snap.docs) {
        ...
        try {
            const result = await model.generateContent(`Startup coach at UNTITLED. Startup: "${s.name}" (${s.industry}, ${s.stage}).
           MRR:$${((_a = s.metrics) === null || _a === void 0 ? void 0 : _a.mrr) || 0} MAU:${((_b = s.metrics) === null || _b === void 0 ? void 0 : _b.mau) || 0} Days stuck:7+
           ...
           Return ONLY JSON: {"hint":"string","category":"product|market|funding|team|legal"}`);
            ...
            if (s.founderEmail) {
                await sendEmail(s.founderEmail, `💡 AI Hint для ${s.name}`, `<div style="max-width:560px;margin:0 auto;padding:32px;background:#050510;font-family:Inter,sans-serif">
            ...
            functions.logger.warn('hint failed for', startupDoc.id, e);
        }
    }
...
// ─────────────────────────────────────────────────────────────────────────────
exports.onStartupMetricUpdate = functions
    .region('us-central1')
    .runWith({ memory: '256MB', timeoutSeconds: 120 })
    .firestore
    ...
    const prevMrr = ((_a = before.metrics) === null || _a === void 0 ? void 0 : _a.mrr) || 0;
    const newMrr = ((_b = after.metrics) === null || _b === void 0 ? void 0 : _b.mrr) || 0;
    const prevRunway = ((_c = before.metrics) === null || _c === void 0 ? void 0 : _c.runwayMonths) || 99;
    const newRunway = ((_d = after.metrics) === null || _d === void 0 ? void 0 : _d.runwayMonths) || 99;
    ...
    let actionPlan = '';
    try {
        const result = await model.generateContent(`Startup "${after.name}" (${after.industry}) has a critical alert: ${isMrrAlert
        ...
        const match = result.response.text().match(/\{[\s\S]*\}/);
        if (match)
            actionPlan = ((_e = JSON.parse(match[0]).steps) === null || _e === void 0 ? void 0 : _e.join(' → ')) || '';
    }
    catch ( /* action plan optional */_f) { /* action plan optional */ }
...
// ─────────────────────────────────────────────────────────────────────────────
exports.founderWeeklyDigest = functions
    .region('us-central1')
    ...
    for (const doc of startupsSnap.docs) {
        ...
        Current MRR: $${((_a = data.metrics) === null || _a === void 0 ? void 0 : _a.mrr) || 0}. Score: ${((_b = data.aiScores) === null || _b === void 0 ? void 0 : _b.overallReadinessScore) || 0}.
        ...
        `;
          ...
          <div style="font-family: sans-serif; max-width: 600px; margin: 0 auto; color: #334155;">
            ...
            <table width="100%" cellpadding="10" style="background: #f8fafc; border-radius: 8px; text-align: center;">
              <tr>
                <td><strong>AI Score</strong><br/>${((_c = data.aiScores) === null || _c === void 0 ? void 0 : _c.overallReadinessScore) || 0}/100</td>
                <td><strong>MRR</strong><br/>$${((_d = data.metrics) === null || _d === void 0 ? void 0 : _d.mrr) || 0}</td>
                <td><strong>MAU</strong><br/>${((_e = data.metrics) === null || _e === void 0 ? void 0 : _e.mau) || 0}</td>
              </tr>
            </table>
        ...
        `;
            for (const fId of founderIds) {
                ...
                if (uDoc.exists && ((_f = uDoc.data()) === null || _f === void 0 ? void 0 : _f.email)) {
