      ...
      <div style={{ display: 'flex', gap: '16px', flex: 1, minHeight: 0 }}>
        ...
        <div style={{ flex: 1, display: 'flex', flexDirection: 'column', gap: '16px' }}>
          ...
          <div style={{ display: 'flex', gap: '16px', height: '180px' }}>
            ...
            {/* Controls */}
            <div style={{ flex: 1, background: 'rgba(0,0,0,0.02)', borderRadius: '16px', border: '1px solid rgba(0,0,0,0.05)', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '16px' }}>
              <button onClick={() => setMicOn(!micOn)} style={{ width: 56, height: 56, borderRadius: '50%', background: micOn ? 'rgba(0,0,0,0.1)' : 'rgba(82,82,91,0.2)', border: `1px solid ${micOn ? 'rgba(0,0,0,0.1)' : 'rgba(82,82,91,0.4)'}`, color: micOn ? '#fff' : '#f87171', display: 'flex', alignItems: 'center', justifyContent: 'center', cursor: 'pointer', transition: 'var(--transition-standard)' }}>
                {micOn ? <Mic size={24} /> : <MicOff size={24} />}
              </button>
              <button onClick={() => setCamOn(!camOn)} style={{ width: 56, height: 56, borderRadius: '50%', background: camOn ? 'rgba(0,0,0,0.1)' : 'rgba(82,82,91,0.2)', border: `1px solid ${camOn ? 'rgba(0,0,0,0.1)' : 'rgba(82,82,91,0.4)'}`, color: camOn ? '#fff' : '#f87171', display: 'flex', alignItems: 'center', justifyContent: 'center', cursor: 'pointer', transition: 'var(--transition-standard)' }}>
        ...
        {/* Sidebar */}
        <div style={{ width: '320px', background: 'rgba(0,0,0,0.02)', borderRadius: '16px', border: '1px solid rgba(0,0,0,0.05)', display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
          {/* Tabs */}
          <div style={{ display: 'flex', borderBottom: '1px solid rgba(0,0,0,0.05)' }}>
            <button onClick={() => setActiveTab('ai')} style={{ flex: 1, padding: '16px', background: activeTab === 'ai' ? 'rgba(0,0,0,0.1)' : 'transparent', border: 'none', color: activeTab === 'ai' ? '#D8B4FE' : '#64748b', fontSize: 13, fontWeight: 600, display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 6, cursor: 'pointer' }}>
              <Brain size={14} /> {t('tabs.aiNotes')}
            </button>
            <button onClick={() => setActiveTab('chat')} style={{ flex: 1, padding: '16px', background: activeTab === 'chat' ? 'rgba(0,0,0,0.05)' : 'transparent', border: 'none', color: activeTab === 'chat' ? '#f8fafc' : '#64748b', fontSize: 13, fontWeight: 600, display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 6, cursor: 'pointer' }}>
          ...
          <div style={{ flex: 1, padding: '16px', overflowY: 'auto' }}>
            {activeTab === 'ai' ? (
              <div style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
                <div style={{ padding: '12px', borderRadius: '10px', background: 'rgba(0,0,0,0.1)', border: '1px solid rgba(0,0,0,0.2)' }}>
